...
def generate_random_rule_body():
    match_type = random.choice(["FIND", "MATCH", "FIND$", "MATCH$"])
    exact = True if "$" in match_type else False
    match_type = match_type.replace("$", "")

    return {
        match_type: {"username": "admin", "exact": exact}
    }
...
def generate_random_rule():
    return {
        "id": str(uuid.uuid4()),
        "name": f"rule_{random.randint(1, 1000)}",
        "body": generate_random_rule_body(),
        "created_at": generate_random_date()
    }


def generate_random_data(number):
    return [{"rule": generate_random_rule()} for _ in range(number)]
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate and inject events into Wazuh Rules index.")
