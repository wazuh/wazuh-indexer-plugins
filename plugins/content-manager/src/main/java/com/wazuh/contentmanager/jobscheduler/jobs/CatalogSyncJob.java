 ...
 * This class handles only scheduling concerns and delegates synchronization to specialized classes.
 */
public class CatalogSyncJob implements JobExecutor {
    ...
    public CatalogSyncJob(Client client, ConsumersIndex consumersIndex, Environment environment, ThreadPool threadPool) {
        ...
        this.synchronizers = List.of(
            new RulesConsumerSynchronizer(client, consumersIndex, environment),
            new DecodersConsumerSynchronizer(client, consumersIndex, environment)
        );
    }
     ...
     * Iterates through all registered synchronizers and executes them.
     */
    private void performSynchronization() {
        for (ConsumerSynchronizer synchronizer : this.synchronizers) {
            try {
                ...
                log.error("Error during synchronization of {}: {}",
                    synchronizer.getClass().getSimpleName(), e.getMessage(), e);
            }
        }
    }
}
