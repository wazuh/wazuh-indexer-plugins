...
public class Ioc {
    private static final Logger log = LogManager.getLogger(Ioc.class);
    ...
    public static Ioc fromPayload(JsonObject payload) {
        Ioc resource = new Ioc();
        Ioc.populateResource(resource, payload);
        return resource;
    }
     ...
     */
    protected static void populateResource(Ioc resource, JsonObject payload) {
        // 1. Process Document
        if (payload.has(JSON_DOCUMENT_KEY) && payload.get(JSON_DOCUMENT_KEY).isJsonObject()) {
            ...
            Ioc.preprocessDocument(rawDoc);

            resource.setDocument(GSON.fromJson(rawDoc, Map.class));

            // 2. Calculate Hash
            String hashStr = Ioc.calculateSha256(rawDoc);
            if (hashStr != null) {
    ...
    /**
     * A non-static wrapper for populateResource (that can be overridden)
     * @param resource A Resource object
     * @param payload The JSON payload
     */
    protected void populateResourceWrapper(Ioc resource, JsonObject payload) {
        Ioc.populateResource(resource, payload);
    }
    ...
    protected static void preprocessDocument(JsonObject document) {
        ...
        if (document.has(JSON_RELATED_KEY)) {
            ...
            if (relatedElement.isJsonObject()) {
                Ioc.sanitizeRelatedObject(relatedElement.getAsJsonObject());
                ...
                for (JsonElement element : relatedArray) {
                    if (element.isJsonObject()) {
                        Ioc.sanitizeRelatedObject(element.getAsJsonObject());
                    }
                }
            }
        }
