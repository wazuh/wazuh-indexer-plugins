...
public class ContentIndexTests extends OpenSearchTestCase {
    ...
    /**
     * Test that operations without `/document` prefix are routed to the inner document node. This
     * covers the CTI sync scenario where patches target paths like `/containers/adp/0/product` but
     * the indexed document wraps content under a `document` key.
     */
    public void testUpdate_OperationRoutedToDocumentNode() throws Exception {
        String id = "CVE-2021-30849";
        ...
        List<Operation> operations = new ArrayList<>();
        operations.add(new Operation("replace", "/containers/adp/0/product", null, "webkitgtk4"));
        ...
        JsonNode updatedDoc = this.mapper.readTree(captor.getValue().source().utf8ToString());
        JsonNode product =
                updatedDoc.get("document").get("containers").get("adp").get(0).get("product");
        Assert.assertNotNull("Product field should exist", product);
        Assert.assertEquals("webkitgtk4", product.asText());
    }

    /**
     * Test that operations with `/document` prefix are applied to the root document. Uses `enabled`
     * field which is preserved as-is through Resource processing (unlike metadata fields like `title`
     * which are moved to `document.metadata`).
     */
    public void testUpdate_OperationWithDocumentPrefix() throws Exception {
        String id = "test-id";
        ...
        List<Operation> operations = new ArrayList<>();
        operations.add(new Operation("replace", "/document/enabled", null, false));
        ...
        Assert.assertFalse(
                ...
                updatedDoc.get("document").get("enabled").asBoolean());
    }

    /**
     * Test that a mix of operations with and without `/document` prefix are both correctly routed
     * within the same update call.
     */
    public void testUpdate_MixedOperationRouting() throws Exception {
        ...
        List<Operation> operations = new ArrayList<>();
        // Path with /document prefix — applied to root currentDoc
        operations.add(new Operation("replace", "/document/enabled", null, false));
        // Path without /document prefix — applied to inner documentNode
        operations.add(new Operation("replace", "/containers/cna/status", null, "updated"));
        ...
        JsonNode updatedDoc = this.mapper.readTree(captor.getValue().source().utf8ToString());
        Assert.assertFalse(updatedDoc.get("document").get("enabled").asBoolean());
        Assert.assertEquals(
                "updated", updatedDoc.get("document").get("containers").get("cna").get("status").asText());
    }
