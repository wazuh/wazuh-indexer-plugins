...
/**
 * {@code GET /_plugins/_setup/ai_assistant/settings} — reads the AI assistant's settings/field
 * policy document in the shared {@code .wazuh-internal-state} system index. Restricted to callers
 * holding the {@code plugin:wazuh/ai_assistant/settings/read} cluster permission.
 */
public class RestGetAiAssistantSettingsAction extends BaseRestHandler {
    private static final String ENDPOINT_NAME = "ai_assistant_get_settings";
    private static final String SETTINGS_URI = "/_plugins/_setup/ai_assistant/settings";

    /** Default constructor. */
    public RestGetAiAssistantSettingsAction() {}
    ...
    public List<Route> routes() {
        return List.of(new Route(RestRequest.Method.GET, SETTINGS_URI));
    }

    @Override
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) {
        GetAiAssistantSettingsRequest getRequest =
                new GetAiAssistantSettingsRequest(Operation.SETTINGS);
        return channel ->
                client.execute(
