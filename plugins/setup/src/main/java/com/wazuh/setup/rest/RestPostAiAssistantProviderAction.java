...
/**
 * {@code POST /_plugins/_setup/ai_assistant/providers} — creates an AI assistant provider document
 * in the shared {@code .wazuh-internal-state} system index. The body must include an {@code id},
 * which must be a UUID. Restricted to callers holding the {@code
 * plugin:wazuh/ai_assistant/settings/write} cluster permission.
 */
public class RestPostAiAssistantProviderAction extends BaseRestHandler {
    private static final String ENDPOINT_NAME = "ai_assistant_post_provider";
    private static final String PROVIDERS_URI = "/_plugins/_setup/ai_assistant/providers";

    /** Default constructor. */
    public RestPostAiAssistantProviderAction() {}
    ...
    public List<Route> routes() {
        return List.of(new Route(RestRequest.Method.POST, PROVIDERS_URI));
    }
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) {
        ...
        PutAiAssistantSettingsRequest putRequest =
                new PutAiAssistantSettingsRequest(Operation.PUT_PROVIDER, null, payload);
        return channel ->
                client.execute(
