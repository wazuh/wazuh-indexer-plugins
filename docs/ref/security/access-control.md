...
|-------------------|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| `wazuh-server`    | User for the Wazuh Server with read/write access to stateful indices and write-only access to stateless indices.                         | `stateless-write`, `stateful-delete`, `stateful-write`, `stateful-read`, `content_manager_server_role`                         |
| `wazuh-dashboard` | User for Wazuh Dashboard with read access to stateful and stateless indices, and management level permissionsfor the monitoring indices. | `sample-data-management`, `metrics-write`, `metrics-read`, `stateless-read`, `stateful-read`, `content_manager_dashboard_role` |
...
| `content_manager_dashboard_role` | Grants permissions to manage subscription endpoints for the dashboard.           | N/A                                      | `plugin:content_manager/subscription_post`, `plugin:content_manager/subscription_delete` |
| `content_manager_server_role`    | Grants permissions to trigger updates and retrieve subscriptions for the server. | N/A                                      | `plugin:content_manager/subscription_get`, `plugin:content_manager/update`               |
