...
| `wazuh-ai-assistant-sessions` | AI assistant conversation history, rotated daily and kept for 7 days. Each user sees only their own conversations (Document Level Security), and writes are mediated by the setup plugin's session API so the owning username cannot be forged. |
