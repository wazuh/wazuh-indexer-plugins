...
| `wazuh‑events-v5-<category>`   | Stores events received by the [Wazuh Manager](https://documentation.wazuh.com/current/getting-started/components/wazuh-server.html), categorized by their origin or type. Refer to [Wazuh Common Schema](./schema.md) for more information. |
| `wazuh‑findings-v5-<category>` | Stores security findings generated by the [Threat Detectors](https://docs.opensearch.org/3.5/security-analytics/#detectors). These are created each time an event trips a detection rule.                                                  |
