...
- **`id`** (String, required) — globally unique rule identifier (UUIDv4). Generated by the Content Manager: leave it out of create and update payloads, where any value supplied is replaced.
- **`status`** (String, required) — rule maturity status: `stable`, `experimental`, `test`, `deprecated` or `unsupported`.
...
A category written as a plain array of ID strings is still accepted, for rules authored against the older documentation:
...
This form carries no ATT&CK names, so `mitre.tactic.name`, `mitre.technique.name` and `mitre.subtechnique.name` are left empty. Use one of the two named forms above for new rules.
